...
Usage:
    train_xgb <tr_data> <mode> [--weights <class_weights>] [--expl <explanation>]

Options:
    -w --weights <class_weights>    If class weights have to be added manually
    -e --expl <explanation>         To add comments, that will be written to the log. Helps keeping track of models.
...
def get_search_params() -> Dict[str, List[Union[None, float, int, str]]]:
    ...
    return {
        ...
        # Tree parameters
        'learning_rate': [0.2, 0.3, 0.4],           # [0.2, 0.3, 0.4],
        'gamma': [1],                     # [0, 1, 3],  # minimum loss reduction required to further partition
        'max_depth': [5, 30, 45],              # [5, 6, 7, 8],  # maximum tree depth
        ...
        'reg_lambda': [1],                # L2 regularizer
        'reg_alpha': [2],                 # L1 regularizer
        ...
        'max_bin': [256, 512],       # only used with 'hist' -- larger -> improve optimality of splits but higher comp. time
...
def get_best_params() -> Dict[str, Union[None, float, int, str]]:
    ...
    TODO RG: check if thats also the best for me: I think this comes from grid_search
    """
    return {'booster': "gbtree",
            ...
            'max_leaves': 0,
            'max_bin': 768, #768 for model with "is blocked"
            'predictor': 'auto',
...
def split_train(X: csr_matrix, y: List[int], weights: Optional[List[float]],
    ...
    evals_result = dict()
    bst: xgb.Booster = xgb.train(params, dtrain, num_boost_round=300, early_stopping_rounds=20,
...
def crossvalidate_train(X, y: Optional[List[int]],
    ...
    """
    Not used at the moment. Could delete it.
    ...
    cv_results = xgb.cv(params, dtrain, num_boost_round=300, nfold=5, stratified=True, metrics=["merror", "mlogloss"],
                        early_stopping_rounds=20, show_stdv=True, verbose_eval=True, seed=random_seed, shuffle=True)
...
def main() -> int:
    ...
    if not os.path.exists(tr_dat_path):
        ...
        return 2
    logger.info(f"using training data from {tr_dat_path}")
    if cargs["--expl"]:
        logger.info(f"{cargs['--expl']}")
    ...
    if cargs["--weights"]:
        class_weights = []
        with open(cargs["--weights"], 'r') as fd:
            for line in fd:
