...
Usage:
    train_xgb.py <tr_data> <mode> [<class_weights>]
...
def get_search_params() -> Dict[str, List[Union[None, float, int, str]]]:
    ...
    return {
        ...
        # Tree parameters
        'learning_rate': [0.2, 0.5],           # [0.2, 0.3, 0.4],
        'gamma': [1],                     # [0, 1, 3],  # minimum loss reduction required to further partition
        'max_depth': [18, 15, 7],                # [5, 6, 7, 8],  # maximum tree depth
        ...
        'colsample_bytree': [1],     # number of features to use, per each tree

        'sampling_method': ['uniform'],
        'reg_lambda': [0, 1, 2],                # L2 regularizer
        'reg_alpha': [0, 1, 2],                 # L1 regularizer
        ...
        'max_bin': [256],                 # only used with 'hist' -- larger -> improve optimality of splits but higher comp. time
