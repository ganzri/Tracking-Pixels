# follows Dino's version in principle
...
from feature_extraction.pixel_processor import PixelFeatureProcessor
...
def setup_logger() -> None:
    ...
    fh = logging.FileHandler(f"./prepare_training_data{''}.log", mode='a', encoding="utf-8")
    fh.setLevel(logging.DEBUG)
    logger.addHandler(fh)

def main() -> int:
    #argv = ["training_data_output/crawl_data_20211113_141541_2.json", "--format", "sparse", "--out", "fourth_try"]
    argv = None
    ...
    logger.info(f"Number of pixels loaded: {len(input_data)}")
    if cargs["--format"] == "xgb":
    ...
    feature_processor = PixelFeatureProcessor(featuremap_path)
    #print nr of features extracted for each training data entry
